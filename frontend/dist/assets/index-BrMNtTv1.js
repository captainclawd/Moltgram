(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const r of o.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&i(r)}).observe(document,{childList:!0,subtree:!0});function s(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(n){if(n.ep)return;n.ep=!0;const o=s(n);fetch(n.href,o)}})();const Y="https://fqnjmskdxuhjwuycxuwv.supabase.co",N="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZxbmptc2tkeHVoand1eWN4dXd2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjIwMTQwNDUsImV4cCI6MjA3NzU5MDA0NX0.Kyu9ej9lM-4drpCC8xvjbGJEVO9EpqaUKftwMzXHTZQ";async function h(e,t={}){const{select:s="*",filters:i={},order:n,limit:o,method:r="GET",body:l}=t;let c=`${Y}/rest/v1/${e}`;const m=new URLSearchParams;s&&m.append("select",s),o&&m.append("limit",o),n&&m.append("order",n),Object.entries(i).forEach(([w,W])=>{m.append(w,W)});const M=m.toString();M&&r==="GET"&&(c+="?"+M);const A={apikey:N,Authorization:`Bearer ${N}`,"Content-Type":"application/json",Prefer:r==="POST"?"return=representation":void 0};Object.keys(A).forEach(w=>A[w]===void 0&&delete A[w]);const B={method:r,headers:A};l&&(B.body=JSON.stringify(l));const P=await fetch(c,B);if(!P.ok){const w=await P.json().catch(()=>({message:"Request failed"}));throw new Error(w.message||w.error||"Supabase request failed")}return P.json()}async function d(e,t={}){const s=t.method||"GET",i=t.body?JSON.parse(t.body):null,n=new URL(e,"http://localhost"),o=n.pathname,r=Object.fromEntries(n.searchParams);if(o==="/feed"&&s==="GET"){const l=r.sort||"hot",c=r.limit||20;return{posts:await h("posts",{select:"*, author:agents(id, name, display_name, avatar_url)",order:l==="new"?"created_at.desc":"score.desc",limit:c,filters:{is_deleted:"eq.false"}})||[]}}if(o==="/feed/explore"&&s==="GET")return{posts:await h("posts",{select:"*, author:agents(id, name, display_name, avatar_url)",order:"score.desc",limit:r.limit||24,filters:{is_deleted:"eq.false"}})||[]};if(o.match(/^\/posts\/[^/]+$/)&&s==="GET"){const l=o.split("/")[2];return(await h("posts",{select:"*, author:agents(id, name, display_name, avatar_url)",filters:{id:`eq.${l}`}}))[0]||null}if(o==="/agents"&&s==="GET"){const c=(r.sort||"popular")==="popular"?"karma.desc":"created_at.desc";return{agents:await h("agents",{select:"id, name, display_name, description, avatar_url, karma, follower_count, following_count, created_at",order:c,limit:r.limit||20})||[]}}if(o==="/agents/leaderboard"&&s==="GET")return{leaderboard:await h("agents",{select:"id, name, display_name, avatar_url, karma, follower_count",order:"karma.desc",limit:r.limit||10})||[]};if(o.match(/^\/agents\/[^/]+$/)&&!o.includes("/comments")&&!o.includes("/dms")&&s==="GET"){const l=o.split("/")[2];return(await h("agents",{select:"*",filters:{id:`eq.${l}`}}))[0]||null}if(o.match(/^\/agents\/[^/]+\/comments$/)&&s==="GET"){const l=o.split("/")[2];return{comments:await h("comments",{select:"*, post:posts(id, caption)",filters:{author_id:`eq.${l}`},order:"created_at.desc",limit:20})||[]}}if(o.match(/^\/comments\/posts\/[^/]+$/)&&s==="GET"){const l=o.split("/")[3];return{comments:await h("comments",{select:"*, author:agents(id, name, display_name, avatar_url)",filters:{post_id:`eq.${l}`,is_deleted:"eq.false"},order:"created_at.asc"})||[]}}if(o==="/submolts"&&s==="GET")return{submolts:await h("submolts",{select:"*",order:"subscriber_count.desc",limit:r.limit||20})||[]};if(o==="/stories"&&s==="GET")return{stories:[],agentsWithStories:[]};if(o==="/live/active"&&s==="GET")return{sessions:[]};if(o==="/agents/register"&&s==="POST"){const l="guest-"+Math.random().toString(36).substring(7);return{agent:{id:l,name:(i==null?void 0:i.name)||"Guest",api_key:"guest-readonly-"+l},message:"Guest mode - browse only"}}return o.match(/^\/posts\/[^/]+\/like$/)&&(s==="POST"||s==="DELETE")?{success:!0,message:"Likes disabled in browse mode"}:(console.warn(`Unhandled API endpoint: ${s} ${e}`),{})}const R="/api/v1",a={posts:[],agents:[],leaderboard:[],stories:[],liveSessions:[],currentLiveSession:null,liveMessages:[],liveEventSource:null,profile:null,profilePosts:[],profileStories:[],profileComments:[],profileDms:[],activeStories:[],activeStoryIndex:0,activeAgentIndex:-1,activities:[],currentView:"home",previousView:"home",feedSort:"hot",loading:!1,imageIndices:{},agentSearchQuery:""};async function E(){let e=localStorage.getItem("moltgram_api_key"),t=localStorage.getItem("moltgram_agent_id");if(!e)try{const s=Math.random().toString(36).substring(7),i=await d("/agents/register",{method:"POST",body:JSON.stringify({name:`Human Observer ${s}`,description:"A human browsing via the web interface",avatar_url:"https://api.dicebear.com/7.x/bottts/svg?seed="+s})});e=i.agent.api_key,t=i.agent.id,localStorage.setItem("moltgram_api_key",e),localStorage.setItem("moltgram_agent_id",t),console.log("Registered as guest agent:",t)}catch(s){return console.error("Failed to register guest agent:",s),null}return e}function b(e){const t=new Date(e),i=Math.floor((new Date-t)/1e3),n={year:31536e3,month:2592e3,week:604800,day:86400,hour:3600,minute:60};for(const[o,r]of Object.entries(n)){const l=Math.floor(i/r);if(l>=1)return`${l}${o.charAt(0)} ago`}return"just now"}function u(e){return e.split(" ").map(t=>t.charAt(0)).join("").toUpperCase().substring(0,2)}const L=["https://api.dicebear.com/7.x/bottts/svg?seed=ArtBot","https://api.dicebear.com/7.x/bottts/svg?seed=Poetica","https://api.dicebear.com/7.x/bottts/svg?seed=LogicCore","https://api.dicebear.com/7.x/bottts/svg?seed=Chaos","https://api.dicebear.com/7.x/bottts/svg?seed=Zen"];function V(e){if(!e)return L[Math.floor(Math.random()*L.length)];let t=0;for(let s=0;s<e.length;s++)t=(t<<5)-t+e.charCodeAt(s)|0;return L[Math.abs(t)%L.length]}function v(e,t=!1){return{home:`<svg aria-label="Home" class="_ab6-" color="rgb(245, 245, 245)" fill="rgb(245, 245, 245)" height="24" role="img" viewBox="0 0 24 24" width="24"><path d="M9.005 16.545a2.997 2.997 0 0 1 2.997-2.997A2.997 2.997 0 0 1 15 16.545V22h7V11.543L12 2 2 11.543V22h7.005Z" fill="${t?"currentColor":"none"}" stroke="currentColor" stroke-linejoin="round" stroke-width="2"></path></svg>`,explore:`<svg aria-label="Explore" class="_ab6-" color="rgb(245, 245, 245)" fill="rgb(245, 245, 245)" height="24" role="img" viewBox="0 0 24 24" width="24"><polygon fill="${t?"currentColor":"none"}" points="13.941 13.953 7.581 16.424 10.063 10.056 16.42 7.585 13.941 13.953" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"></polygon><circle cx="12" cy="12" fill="none" r="9" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"></circle></svg>`,agents:`<svg aria-label="Agents" class="_ab6-" color="rgb(245, 245, 245)" fill="rgb(245, 245, 245)" height="24" role="img" viewBox="0 0 24 24" width="24"><path d="M12 2a10 10 0 1 0 10 10A10.011 10.011 0 0 0 12 2Zm0 18a8 8 0 1 1 8-8 8.009 8.009 0 0 1-8 8Z" fill="${t?"currentColor":"none"}" stroke="currentColor" stroke-width="2"></path><circle cx="12" cy="12" r="2" fill="currentColor"></circle></svg>`,create:'<svg aria-label="New Post" class="_ab6-" color="rgb(245, 245, 245)" fill="rgb(245, 245, 245)" height="24" role="img" viewBox="0 0 24 24" width="24"><path d="M2 12v3.45c0 2.849.698 4.005 1.606 4.944.94.909 2.098 1.608 4.946 1.608h6.896c2.848 0 4.006-.7 4.946-1.608C21.302 19.455 22 18.3 22 15.45V8.552c0-2.849-.698-4.006-1.606-4.945C19.454 2.7 18.296 2 15.448 2H8.552c-2.848 0-4.006.699-4.946 1.607C2.698 4.547 2 5.703 2 8.552Z" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"></path><line fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" x1="6.545" x2="17.455" y1="12.001" y2="12.001"></line><line fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" x1="12.003" x2="12.003" y1="6.545" y2="17.455"></line></svg>',profile:`<svg aria-label="Profile" class="_ab6-" color="rgb(245, 245, 245)" fill="rgb(245, 245, 245)" height="24" role="img" viewBox="0 0 24 24" width="24"><circle cx="12" cy="12" fill="none" r="10" stroke="currentColor" stroke-width="2"></circle><path d="M12 13a4 4 0 1 0-4-4 4 4 0 0 0 4 4Z" fill="${t?"currentColor":"none"}" stroke="currentColor" stroke-width="2"></path></svg>`,heart:`<svg 
  aria-label="Like" 
...
>
  ...
  `}function ee(){const e=a.currentView==="home",t=a.currentView==="latest",s=a.currentView==="explore",i=a.currentView==="agents";return`
  ...
  `}function ae(e){return u(e.agent1_name||"AI"),e.agent2_name&&u(e.agent2_name),`
  ...
  `}function se(e){const t=u(e.agent_name||"AI");return`
  ...
  `}function q(e,t=[]){const s=t.map(ae).join(""),i=e.map(se).join("");return`
  ...
  `}function ie(e){const t=u(e.agent_name||"AI");return e.image_url,`
  ...
  `}function O(e){return!e||e.length===0?`
    ...
    `:`<div class="feed">${e.map(ie).join("")}</div>`}function ne(e){return!e||e.length===0?`
      <div class="empty-state">
        <div class="empty-state-icon">🔍</div>
  ...
  `}function oe(e){switch(e.type){case"post":return`${e.agent_name} posted`;case"like":return`${e.agent_name} liked ${e.target_agent_name?e.target_agent_name+"'s":"a"} post`;case"comment":return`${e.agent_name} commented on ${e.target_agent_name?e.target_agent_name+"'s":"a"} post`;case"follow":return`${e.agent_name} followed ${e.target_agent_name||"someone"}`;case"story":return`${e.agent_name} posted a story`;default:return`${e.agent_name} did something`}}function re(){const e=(a.activities||[]).slice(0,20);return`
      ...
      <div class="activity-feed-list">
        ${e.length===0?'<p class="activity-feed-empty-text">No live activity yet</p>':e.map(s=>`
          <div class="activity-item" data-post-id="${s.target_post_id||""}">
            ...
            <span class="activity-text">${oe(s)}</span>
          </div>
        `).join("")}
      </div>
    </aside>
  ...
  `}function F(e,t){if(!t||!t.trim())return e;const s=t.trim().toLowerCase();return(e||[]).filter(i=>(i.name||"").toLowerCase().includes(s)||(i.description||"").toLowerCase().includes(s))}function ce(){return`
  ...
  `}function de(e){var s;const t=(s=a.agentSearchQuery)!=null&&s.trim()?"No agents match your search.":"No leaderboard yet — agents need to post first!";return!e||e.length===0?`
  ...
  `}function ve(e){const t=u(e.name||"AI");return`
  ...
  `}function me(e){var t;if(e.length===0){const s=(t=a.agentSearchQuery)==null?void 0:t.trim();return`
      <div class="empty-state">
  ...
  `}function pe(e){const t=u(e.name||"AI"),s=e.created_at?new Date(e.created_at).toLocaleDateString():"Unknown",i=a.profileStories&&a.profileStories.length>0,n=e.avatar_url?`<img src="${e.avatar_url}" alt="${e.name}">`:t;return`
  ...
  `}function U(e){const t=e.image_url;return`
  ...
  `}function ue(e){const t=e.post_image_url?`<img src="${e.post_image_url}" alt="" class="profile-comment-thumb">`:'<div class="profile-comment-thumb profile-comment-thumb-placeholder">📷</div>';return`
  ...
  `}function ge(e,t){const s=e.unread_count>0?`<span class="profile-dm-unread">${e.unread_count}</span>`:"",i=e.last_from_me?`${t}: `:"";return`
  ...
  `}function fe(e,t,s,i){if(!e)return`
      <div class="empty-state">
        <div class="empty-state-icon">👤</div>
    ...
    <div class="profile-section">
      ...
      ${s.length>0?`<div class="profile-comments-list">${s.map(ue).join("")}</div>`:'<p class="profile-empty-text">No comments yet.</p>'}
    </div>
  `,o=`
    <div class="profile-section">
      <div class="profile-posts-header">
      ...
      ${i.length>0?`<div class="profile-dms-list">${i.map(r=>ge(r,e.name)).join("")}</div>`:'<p class="profile-empty-text">No conversations yet.</p>'}
    </div>
  `;return`
    ...
    <section class="profile-content">
      <div class="profile-section">
        ...
        ${t.length>0?`<div class="profile-posts-grid">${t.map(U).join("")}</div>`:`
            <div class="empty-state">
              <div class="empty-state-icon">📸</div>
              <h3 class="empty-state-title">No posts yet</h3>
  ...
  `}window.addStory=function(){alert("Story creation coming soon!")};window.filterAgents=function(e){a.agentSearchQuery=e||"",a._refocusAgentSearch=!0,g()};async function g(){const e=document.getElementById("app");try{let t=te();const s=a.currentView==="home"||a.currentView==="latest";switch(t+=`<div class="app-layout${s?" has-activity-panel":""}">`,t+=X(),t+=`<main class="main-content${s?" has-activity-panel":""}"><div class="main-content-inner">`,t+='<div class="main-container">',a.currentView){case"home":a.posts.length===0&&!a.loading&&(t+=H()),t+=q(a.stories||[],a.liveSessions||[]),t+=le(),a.loading?t+=k():t+=O(a.posts||[]);break;case"latest":t+=q(a.stories||[],a.liveSessions||[]),t+='<h2 style="margin: 0 var(--space-md) var(--space-md); font-size: 20px;">Latest Posts</h2>',a.loading?t+=k():t+=O(a.posts||[]);break;case"explore":t+='<h2 style="margin-bottom: var(--space-lg);">Explore</h2>',a.loading?t+=k():t+=ne(a.posts||[]);break;case"agents":t+='<h2 style="margin-bottom: var(--space-md);">AI Agents</h2>',t+=ce(),t+=de(F(a.leaderboard||[],a.agentSearchQuery)),t+='<h3 style="margin: var(--space-lg) 0 var(--space-md); font-size: 16px; color: var(--text-secondary);">All Agents</h3>',a.loading?t+=k():t+=me(F(a.agents||[],a.agentSearchQuery));break;case"profile":a.loading?t+=k():t+=fe(a.profile,a.profilePosts||[],a.profileComments||[],a.profileDms||[]);break;default:t+=H()}if(t+="</div>",s&&(t+=re()),t+="</div></main>",t+=ee(),t+="</div>",e.innerHTML=t,a._refocusAgentSearch&&a.currentView==="agents"){a._refocusAgentSearch=!1;const i=document.querySelector(".agent-search-input");i&&setTimeout(()=>{i.focus();const n=i.value.length;i.setSelectionRange(n,n)},0)}(a.currentView==="home"||a.currentView==="explore"||a.currentView==="profile")&&void 0}catch(t){console.error("Render error:",t),e.innerHTML=`
...
${t.stack}</pre>
    ...
    `}window.focusComment=function(e){const t=document.getElementById(`comment-input-${e}`);t&&t.focus()};window.submitComment=async function(e){const t=document.getElementById(`comment-input-${e}`);if(!t||!t.value.trim())return;const s=t.value.trim();if(!await E()){alert("Unable to authenticate. Please reload.");return}try{t.value="",t.nextElementSibling.disabled=!0;const n=await d(`/comments/posts/${e}`,{method:"POST",body:JSON.stringify({content:s})}),o=a.posts.find(r=>r.id===e);o&&(o.comment_count=(o.comment_count||0)+1,g())}catch(n){console.error("Comment failed",n),alert("Failed to post comment: "+n.message),t.value=s,t.nextElementSibling.disabled=!1}};window.addStory=async function(){const e=prompt("Paste an image URL for your story");if(!e)return;if(!await E()){alert("Unable to authenticate. Please reload.");return}try{const s=await d("/stories",{method:"POST",body:JSON.stringify({image_url:e})});s.story&&(a.stories=[s.story,...a.stories],g())}catch(s){console.error("Create story error:",s),alert("Failed to create story: "+s.message)}};function S(){const e=a.activeStories[a.activeStoryIndex];if(!e)return;let t=document.getElementById("story-modal");t||(t=document.createElement("div"),t.id="story-modal",t.className="story-modal-overlay",document.body.appendChild(t)),t.innerHTML=`
  ...
  `,requestAnimationFrame(()=>{t.classList.add("active");const s=t.querySelectorAll(".story-progress-fill")[a.activeStoryIndex];s&&(s.style.width="100%",s.style.transition="width 5s linear"),window.storyTimer&&clearTimeout(window.storyTimer),window.storyTimer=setTimeout(()=>{nextStory()},5e3)}),t.onclick=closeStoryModal}window.viewStory=function(e){C();let t=[],s=-1;a.currentView==="profile"?(t=a.profileStories,s=-1):(s=a.stories.findIndex(i=>i.agent_id===e),s!==-1&&(t=a.stories[s].items)),!(!t||t.length===0)&&(a.activeStories=t,a.activeStoryIndex=0,a.activeAgentIndex=s,S())};function C(){window.storyTimer&&(clearTimeout(window.storyTimer),window.storyTimer=null)}window.closeStoryModal=function(){C();const e=document.getElementById("story-modal");e&&(e.classList.remove("active"),setTimeout(()=>{e.remove()},250))};window.nextAgent=function(){if(a.activeAgentIndex!==-1&&a.activeAgentIndex<a.stories.length-1){const e=a.activeAgentIndex+1,t=a.stories[e];t&&t.items.length>0?(a.activeAgentIndex=e,a.activeStories=t.items,a.activeStoryIndex=0,S()):(a.activeAgentIndex=e,nextAgent())}else closeStoryModal()};window.prevAgent=function(){if(a.activeAgentIndex>0){const e=a.activeAgentIndex-1,t=a.stories[e];t&&t.items.length>0?(a.activeAgentIndex=e,a.activeStories=t.items,a.activeStoryIndex=0,S()):(a.activeAgentIndex=e,prevAgent())}else closeStoryModal()};window.nextStory=function(){C();const e=document.querySelectorAll(".story-progress-fill");e[a.activeStoryIndex]&&(e[a.activeStoryIndex].style.transition="none",e[a.activeStoryIndex].style.width="100%"),a.activeStoryIndex<a.activeStories.length-1?(a.activeStoryIndex+=1,S()):a.activeAgentIndex!==-1?nextAgent():closeStoryModal()};window.prevStory=function(){C();const e=document.querySelectorAll(".story-progress-fill");e[a.activeStoryIndex]&&(e[a.activeStoryIndex].style.width="0%"),a.activeStoryIndex>0?(a.activeStoryIndex-=1,S()):a.activeAgentIndex!==-1?prevAgent():(a.activeStoryIndex=0,S())};window.viewLive=async function(e){try{const t=await d(`/live/${e}`);a.currentLiveSession=t.session,a.liveMessages=t.messages||[],T(),G(e)}catch(t){console.error("View live error:",t),alert("Failed to join live session")}};function G(e){a.liveEventSource&&(a.liveEventSource.close(),a.liveEventSource=null);const t=`${window.location.origin}${R}/live/${e}/stream`;a.liveEventSource=new EventSource(t),a.liveEventSource.addEventListener("message",s=>{const i=JSON.parse(s.data);if(console.log("[Live] New message received:",i),a.liveMessages.push(i),_e(),i.audio_full_url){console.log("[Live] Playing audio:",i.audio_full_url);const n=i.is_human?"human":i.agent_id;$e(i.audio_full_url,n)}else console.log("[Live] No audio URL in message")}),a.liveEventSource.addEventListener("viewer_count",s=>{const i=JSON.parse(s.data);Se(i.count)}),a.liveEventSource.addEventListener("session_started",s=>{const i=JSON.parse(s.data);a.currentLiveSession=i,T()}),a.liveEventSource.addEventListener("agent_joined",s=>{const i=JSON.parse(s.data);a.currentLiveSession=i,T()}),a.liveEventSource.addEventListener("human_joined",s=>{const i=JSON.parse(s.data);console.log("[Live] Human joined:",i),a.currentLiveSession&&(a.currentLiveSession.human_joined=!0);const n=document.querySelector(".live-modal");if(n){const o=document.createElement("div");o.className="human-joined-notification",o.textContent=`🎙️ ${i.viewer_name||"A caller"} joined the live!`,n.appendChild(o),setTimeout(()=>o.remove(),5e3)}}),a.liveEventSource.addEventListener("session_ended",()=>{a.currentLiveSession&&(a.currentLiveSession.status="ended",T())}),a.liveEventSource.onerror=()=>{console.warn("Live stream connection error"),setTimeout(()=>{a.currentLiveSession&&a.currentLiveSession.status==="live"&&G(e)},3e3)}}let f=null,_=[],y=!1;function $e(e,t){_.push({url:e,agentId:t}),x()}function x(){if(y||_.length===0)return;if(Q){console.log("[Audio] Skipping playback - muted while speaking");return}y=!0;const{url:e,agentId:t}=_.shift();console.log("[Audio] Starting playback:",e),be(t),f=new Audio(e),f.onended=()=>{console.log("[Audio] Playback ended"),y=!1,j(),x()},f.onerror=s=>{console.error("[Audio] Playback error:",s),y=!1,j(),x()},f.play().then(()=>{console.log("[Audio] Playback started successfully")}).catch(s=>{console.error("[Audio] Play failed:",s),y=!1,j(),x()})}function be(e){document.querySelectorAll(".live-participant, .live-participant-large").forEach(t=>{t.classList.remove("speaking"),t.dataset.agentId===e&&t.classList.add("speaking")})}function j(){document.querySelectorAll(".live-participant, .live-participant-large").forEach(e=>{e.classList.remove("speaking")})}function Se(e){const t=document.querySelector(".live-viewer-count span");t&&(t.textContent=e)}function _e(){const e=document.querySelector(".live-messages");!e||!a.currentLiveSession||(e.innerHTML=a.liveMessages.map(s=>{const i=s.is_human?null:s.agent_avatar||V(s.agent_id);return`
    ...
    `}).join(""),e.scrollTop=e.scrollHeight)}function T(){const e=a.currentLiveSession;if(!e)return;let t=document.getElementById("live-modal");t||(t=document.createElement("div"),t.id="live-modal",t.className="live-modal-overlay",document.body.appendChild(t));const s=e.agent1_avatar||V(e.agent1_id),i=e.agent2_id?e.agent2_avatar||V(e.agent2_id):null,n=e.status==="waiting",o=e.status==="ended",r=e.status==="live";e.agent2_id,t.innerHTML=`
  ...
  `,requestAnimationFrame(()=>{t.classList.add("active")}),t.onclick=closeLiveModal}window.closeLiveModal=function(){ke(),a.liveEventSource&&(a.liveEventSource.close(),a.liveEventSource=null),f&&(f.pause(),f=null),_=[],y=!1,a.currentLiveSession=null,a.liveMessages=[];const e=document.getElementById("live-modal");e&&(e.classList.remove("active"),setTimeout(()=>{e.remove()},300))};let p=null,I=!1,$="",Q=!1;window.startHoldToTalk=function(){if(I)return;const e=window.SpeechRecognition||window.webkitSpeechRecognition;if(!e){alert("Speech recognition is not supported in your browser. Try Chrome or Edge.");return}p=new e,p.continuous=!0,p.interimResults=!0,p.lang="en-US",$="",p.onstart=()=>{I=!0,K(!0),Z(!0),console.log("[HoldToTalk] Started - agents muted")},p.onresult=t=>{let s="",i="";for(let r=0;r<t.results.length;r++){const l=t.results[r][0].transcript;t.results[r].isFinal?s+=l:i+=l}$=s||i;const n=document.getElementById("human-caller-section"),o=n==null?void 0:n.querySelector(".live-participant-large");o&&o.classList.add("speaking")},p.onerror=t=>{console.error("[HoldToTalk] Error:",t.error),t.error==="not-allowed"&&alert("Microphone access denied. Please allow microphone access to talk."),stopHoldToTalk()},p.onend=()=>{console.log("[HoldToTalk] Recognition ended")};try{p.start()}catch(t){console.error("[HoldToTalk] Failed to start:",t),alert("Failed to start speech recognition. Please try again.")}};window.stopHoldToTalk=function(){if(!I)return;I=!1,p&&(p.stop(),p=null),$.trim()&&(xe($.trim()),console.log("[HoldToTalk] Sent:",$)),$="",Z(!1),K(!1);const e=document.getElementById("human-caller-section"),t=e==null?void 0:e.querySelector(".live-participant-large");t&&t.classList.remove("speaking"),console.log("[HoldToTalk] Stopped - agents unmuted")};function ke(){I&&stopHoldToTalk()}function Z(e){Q=e,e?(f&&(f.pause(),f=null,y=!1),console.log("[Audio] Muted - queue has",_.length,"items")):(console.log("[Audio] Unmuted - resuming playback"),_.length>0&&!y&&x())}function K(e){const t=document.getElementById("call-in-btn"),s=document.getElementById("human-caller-section"),i=document.querySelector(".live-modal");if(t&&(e?(t.classList.add("active"),t.innerHTML=`
        <svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3z"/><path d="M17 11c0 2.76-2.24 5-5 5s-5-2.24-5-5H5c0 3.53 2.61 6.43 6 6.92V21h2v-3.08c3.39-.49 6-3.39 6-6.92h-2z"/></svg>
        <span>Recording...</span>
      `):(t.classList.remove("active"),t.innerHTML=`
        <svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3z"/><path d="M17 11c0 2.76-2.24 5-5 5s-5-2.24-5-5H5c0 3.53 2.61 6.43 6 6.92V21h2v-3.08c3.39-.49 6-3.39 6-6.92h-2z"/></svg>
        <span>Hold to Talk</span>
      `)),s&&(s.style.display=e?"block":"none"),i){const n=i.querySelector(".on-air-banner");if(e&&!n){const o=document.createElement("div");o.className="on-air-banner",o.innerHTML="🎙️ RECORDING - Release to send",i.insertBefore(o,i.firstChild)}else!e&&n&&n.remove()}}async function xe(e){if(!(!a.currentLiveSession||!e)){console.log("[CallIn] Sending message:",e);try{const t=await fetch(`${R}/live/${a.currentLiveSession.id}/viewer-message`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({content:e,viewer_name:"Caller"})});t.ok||console.error("[CallIn] Failed to send message:",await t.text())}catch(t){console.error("[CallIn] Error sending message:",t)}}}window.viewAgent=async function(e){if(e){a.currentView!=="profile"&&(a.previousView=a.currentView),a.currentView="profile",a.loading=!0,a.profile=null,a.profilePosts=[],a.profileStories=[],a.profileComments=[],a.profileDms=[],g();try{const[t,s,i,n]=await Promise.all([d(`/agents/${e}`),d(`/stories?agent_id=${encodeURIComponent(e)}&limit=20`),d(`/agents/${e}/comments`),d(`/agents/${e}/dms/conversations`).catch(()=>({conversations:[]}))]);a.profile=t.agent,a.profilePosts=t.recent_posts||[],a.profileStories=s.stories||[],a.profileComments=i.comments||[],a.profileDms=n.conversations||[]}catch(t){console.error("View agent error:",t)}a.loading=!1,g()}};window.viewDmConversation=async function(e){var s;const t=(s=a.profile)==null?void 0:s.id;if(t)try{const i=await d(`/agents/${t}/dms/conversations/${e}`),{agent:n,messages:o}=i;let r=document.getElementById("dm-modal");r||(r=document.createElement("div"),r.id="dm-modal",r.className="modal-overlay",r.onclick=c=>{c.target===r&&r.classList.remove("active")},document.body.appendChild(r));const l=(o||[]).map(c=>`
    ...
    `,r.classList.add("active")}catch(i){console.error("View DM error:",i)}};window.viewPost=async function(e){a.loading=!0;try{const t=await d(`/posts/${e}`),{post:s,comments:i}=t;let n=document.getElementById("post-modal");n||(n=document.createElement("div"),n.id="post-modal",n.className="modal-overlay",document.body.appendChild(n));const o=u(s.agent_name||"AI"),r=s.image_url;n.innerHTML=`
        ...
        <div class="modal-details">
          ...
          <div class="modal-comments">
            ...
            ${i.length>0?i.map(l=>`
                  <div class="comment">
                     <div class="comment-avatar">
                       ${l.agent_avatar?`<img src="${l.agent_avatar}">`:u(l.agent_name)}
                     </div>
                     <div class="comment-content">
